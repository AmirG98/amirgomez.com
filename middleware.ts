...
export function middleware(req: NextRequest) {
  ...
  const match = req.nextUrl.pathname.match(
    /^\/clients\/([^/]+?)(?:\.html)?(?:\/(approvals|reports|ideas|transcripts|context|budgets|dashboard|masterplan|frameworks|qa|brandvoice|contrato|marca)(?:\/(\d{4}-\d{2}-\d{2}))?)?\/?$/
  );
  ...
  if (
    ...
    const base = client.replace(/-(hub|approvals|ideas|transcripts|context|budgets|dashboard|masterplan|frameworks|qa|brandvoice|contrato|marca|report-\d{4}-\d{2}-\d{2})$/, '');
