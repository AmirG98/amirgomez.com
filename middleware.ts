...
export function middleware(req: NextRequest) {
  const match = req.nextUrl.pathname.match(/^\/clients\/([^/]+?)(?:\.html)?(?:\/(approvals|reports|ideas))?\/?$/);
  if (!match) return NextResponse.next();

  const client = match[1];
  const section = match[2];

  // Archivos internos (-hub, -approvals, -ideas) nunca se sirven por URL directa; -login sí.
  if (client.endsWith('-login')) return NextResponse.next();
  if (client.endsWith('-hub') || client.endsWith('-approvals') || client.endsWith('-ideas')) {
    const base = client.replace(/-(hub|approvals|ideas)$/, '');
